...
@property (assign) NSUInteger cropType;
@end
...
- (void) cropImage: (CDVInvokedUrlCommand *) command {
    ...
    self.cropType = options[@"cropType"] ? [options[@"cropType"] intValue] : 100;
    
    
    NSLog(@"Value of cropType = %lu", (unsigned long)self.cropType);
    ...
    if(self.cropType == 1){
        cropController.customZoom = 0.0f;
        cropController.imageCropRect = CGRectMake((width - length) / 2,(height - length) / 2,length,length/2);
        
    }else if(self.cropType == 2){
    
    //    Update Pinch To Zoom
    cropController.customZoom = 20.0f;
    cropController.imageCropRect = CGRectMake(0,0,width,height);
    
    }

//    cropController.imageCropRect = CGRectMake((width - length) / 2,(height - length) / 2,length,length);
